...
public class ReviewServiceImpl implements ReviewService {
	...
	@Override
	public ReviewUpdateResponseDto updateReview(ReviewUpdateDto reviewUpdateRequest) throws ReviewException {
		logger.debug("In update review method, calling repo");
		Optional<Review> previouslyPersistedData = reviewRepository.findById(reviewUpdateRequest.getReviewId());

		if(!previouslyPersistedData.isPresent()) {
			throw new ReviewException("Update Failed, respective review details not found");
		}
		if(!(reviewUpdateRequest.getRestaurantId().equals(previouslyPersistedData.get().getRestaurantId() ))) {
		throw new ReviewException("Update Failed, cannot change Restaurant details while updating rating/review");

	}
		Review review = new Review(reviewUpdateRequest.getReviewDetails(), reviewUpdateRequest.getRating(), previouslyPersistedData.get().getCustomerId(), reviewUpdateRequest.getOrderId(), reviewUpdateRequest.getRestaurantId());
		review.setId(previouslyPersistedData.get().getId());
		Review savedReview = reviewRepository.save(review);
		return new ReviewUpdateResponseDto(savedReview.getId(), savedReview.getReviewDetails(), savedReview.getRating(), savedReview.getOrderId(), savedReview.getRestaurantId(), savedReview.getCustomerId());
